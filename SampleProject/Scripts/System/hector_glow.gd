...
func _process(delta: float) -> void:
	...
	if not enable_glow and not get_parent().hit_effect_applied:
		...
		elif Global.player.stats.status[Global.player.stats.Status.RESIST_FIRE_ICE] > 0:
			editShaderParams(0.1, 4, true, Color(1, 0, 1))
		elif Global.player.drowning:
			editShaderParams(0.15, 6, true, Color(0, 0.3, 1))
		else:
